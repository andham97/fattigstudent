...
router.get('/', function (req, res, next) {
    ...
    pool.getConnection(function (err, connection) {
        ...
        if (req.query.roulette != null) {
            i = 0;
            sql = "SELECT Varenavn, Varetype, Alkohol, Pris, Volum, PPK, Vareurl FROM PPK";
            val = {sql: sql, i: i};
        ...
        if (req.query.search == undefined) {
            connection.query("SELECT Varenavn, Varetype, Alkohol, Pris, Volum, PPK, Vareurl FROM PPK LIMIT 10", function (err, rows) {
                connection.release();
                if (!err) {
                    if (rows.length == 0)
        ...
        if (val.i == 0) {
            if(typeof req.query.search == "string" && req.query.length > 0)
                val.sql += " WHERE Varenavn LIKE '%" + req.query.search + "%'";
            else
                val.sql += " WHERE 1 = 1";
            if (req.query.vmin != null && req.query.vmax != null && req.query.vmin != "" && req.query.vmax != "") {
                ...
                if (!err) {
                    if (rows.length == 0)
                        ...
                        for (var i = 0; i < rows.length; i++) {
                            ...
                            s += "<tr bgColor='" + (i % 2 == 0 ? "#ffffff" : "#f2f2f2") + "'><td><a target='_blank' href='" + rows[i].Vareurl + "'>" + rows[i].Varenavn + "</a></td><td>" + rows[i].Varetype + "</td><td>" + rows[i].Alkohol + "%</td><td>" + rows[i].Pris + "kr</td><td>" + rows[i].Volum + "L</td><td>" + rows[i].PPK + "</td></tr>";
                        }
                        s += "</table>";
...
function addSQLSearch(val, vt, req) {
    ...
    val.sql += " AND Varenavn LIKE '%" + req.query.search + "%'";
    if (req.query.vmin != null && req.query.vmax != null && req.query.vmin != "" && req.query.vmax != "") {
        val.sql += " AND Volum BETWEEN '" + req.query.vmin + "' AND '" + req.query.vmax + "'";
    }
    if (req.query.vmin != null && req.query.vmax != null && req.query.vmin != "" && req.query.vmax == "") {
